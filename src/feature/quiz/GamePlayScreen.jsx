...
const GamePlayScreen = () => {
  ...
  const {question, options, answer, explanation, currentQuestionData, setCurrentQuestionData} = useQuizCurrentQuestion(activeQuestionData);
    ...
    <div className="flex flex-col gap-6">
      <div className="flex justify-between" data-roles="game-header">
        <div className="flex items-center gap-3">
          <p className="text-white">Question {currentQuestionData + 1} of {activeQuestionData.length}</p>
          <div>
      ...
      <div className="flex justify-between" data-roles="game-footer">
        ...
        <div className="flex items-center gap-4">
          <button onClick={handleQuizSubmit} className="bg-green-600/50 flex items-center gap-2 text-white text-sm outline-1 py-1 px-4 rounded-sm cursor-pointer hover:bg-green-600/60">
